        ...
        <form action="{{ route("admin.sales.update", [$sale->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <table class="table table-bordered" id="product_info_table">
                 ...
                 <tbody id="product-info">
                       <!-- Product info  -->
                       @foreach($sale->products as $id => $pd)
                       <tr id="row_{{ $id + 1}}" class="product">
                            <td>
                                 <select class="form-control" id="product_{{ $id + 1}}" name="products[]" style="width:100%;" onchange="getProductData({{ $id + 1 }})" required>
                                      <option value=""> {{ trans('global.pleaseSelect') }} </option>
                                      @foreach($products as  $product)
                                      <option value="{{ $product->id }}" {{ (isset($sale) && $sale->products ? $sale->products[$id]->id : old('product_id')) == $product->id ? 'selected' : '' }} >
                                           {{ $product->name }}
                                      </option>
                                      @endforeach
                                 </select>
                            ...
                            <td>
                                 <input type="number" min="1" name="quantities[]" id="quantity_{{ $id + 1}}" class="form-control" value="1" required onchange="getTotal(1)" onkeyup="getTotal(1)">
                            </td>
                            <td>
                                 <input type="number" id="rate_{{ $id + 1}}" value="{{ $pd->price }}" name="rates[]" class="form-control" readonly>
                            </td>
                            <td>
                                 <input type="number" name="price[]" id="price_{{ $id + 1}}" value="{{ $pd->price }}" class="form-control price" readonly>
                            </td>
                            <td>
            ...
            <div class="form-row">
                 ...
                 <div class="col-6">
                    ...
                    <input type="number" step="any" min="0" id="discount" name="discount" class="form-control" value="{{ $sale->discount ?? 0 }}" onchange="getAmount()" onkeyup="getAmount()">
