...
@can('income_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.incomeCategories.create") }}">
                        ...
                        <tr data-entry-id="{{ $category->id }}">
                            ...
                            <td>
                                @can('income_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.incomeCategories.show', $category->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('expense_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.incomeCategories.edit', $category->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('expense_category_delete')
                                    <form action="{{ route('admin.incomeCategories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
<script>
    $(function () {
         ...
         let deleteButton = {
             ...
             url: "{{ route('admin.incomeCategories.massDestroy') }}",
